...
def plot(graph,x_var,x_label,model,x_plot,title,est_names,permute=False):
    ...
    if model == 'deg1':
        # Compute MSE as average of squared relative biases but exclude DM estimators from plot
        df = df.loc[(df["Estimator"]!='DM') & (df['Estimator']!='DM($0.75$)')]
        df["biassq"] = df["Bias"]**2
        df2 = df.groupby([x_var,'Estimator']).agg('mean')

        # Plot
        sns.lineplot(x=x_var, y='biassq', hue='Estimator', style='Estimator', data=df2, palette = ['#2ba02c','#d62728','#9468bd'], legend='brief', dashes=[(1,1),(3,2,2,2),(4,1,1,1)], markers=['s','P','D'])
        ax.set_ylim(0,0.05)
    else:
        # Compute MSE
        df["biassq"] = df["Bias"]**2
        ...
        # Plot
        sns.lineplot(x=x_var, y='biassq', hue='Estimator', style='Estimator', data=df2, legend='brief', markers=True)
        ax.set_ylim(0,0.5)
